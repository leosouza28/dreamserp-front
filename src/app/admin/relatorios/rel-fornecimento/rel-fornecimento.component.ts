...
@Component({
  selector: 'app-rel-fornecimento',
  templateUrl: './rel-fornecimento.component.html',
  styleUrls: ['./rel-fornecimento.component.scss']
})
export class RelFornecimentoComponent {
  form!: FormGroup;
  ...
  constructor(private endpointService: EndpointsService, private alert: AlertService, private fb: FormBuilder) {
    this.form = this.fb.group({
      id: this.fb.control('relatorio-fornecimento-01'),
      param_data: this.fb.control('data_venda'),
      cliente: this.fb.control("TODOS"),
